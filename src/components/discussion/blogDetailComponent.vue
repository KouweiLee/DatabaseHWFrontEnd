                    ...
                    <el-col>
                        ...
                        <el-card style="border: none" shadow="never">
                            ...
                            <el-row>
                                ...
                                <span style="font-size: 10px; color: gray;margin: auto 20px;">发布于{{
                                        blogInfo.time
                                    }}</span>
                                        ...
                                        <el-card shadow="never" style="margin-top: 0;border: none">
                                            <template #header>
                                                <el-row>
                                                    ...
                                                    <span class="name">{{ item.commentator }}({{ item.member_type }})回复: {{ item.beCommentator }}</span>
                                                </el-row>
                                            </template>
                                            ...
                                            <el-row>
                                                <!--回复下方的一些功能，包括时间，评论和删除-->
                                                <span style="font-size: 10px; color: gray;margin: auto 20px;">发布于{{item.time }}</span>
                                                <div class="spacer"></div>
                                                <el-button type="primary" :icon="Edit" @click="showAddCommentForm(item.commentator)" circle/>
                                                <el-button type="danger" :icon="Delete" @click="showDeleteCommentWindow(item.comment_id, item.commentator)" circle/>
                                            </el-row>
                                        </el-card>
                                    </el-col>
                                </el-row>
...
export default {
    ...
    setup() {
            ...
            API.post(API.defaults.baseUrl + '/discuss/queryone/',
                ...
                if (response.data.code === 200) {
                    ...
                    for (let [index, item] of response.data.data.comments.entries()) {
                        ...
                        obj.time = item.time
                        obj.commentator = item.commentator_name
                        obj.beCommentator = item.beCommentator_name
        ...
        function showDeleteCommentWindow(comment_id, commentator) {
            if (STORE.state.userName === commentator) {
            ...
            API.post(API.defaults.baseUrl + "/discuss/addcomment/",
                ...
                if (response.data.code === 200) {
                    ...
                    blogInfo.comments.push({
                        ...
                        commentator: STORE.state.userName,
